    ...
    internal class ApplicationUserService : IApplicationUserService
    {
        ...
        public async Task<Result> DeleteUserAsync(string userId) {
            ...
            try {
                ...
                if (result.Succeeded) {
                    ...
                    serviceResult.Fail().WithMessage($"Unable to delete user.");
                    _logger.LogWarning($"Unable to delete user '{userId}'");
                }
            }
            catch (Exception ex) {
        ...
        public async Task<Result> ActivateUserAsync(string username) {
            ...
            try {
                ...
                if (result.Succeeded) {
                    serviceResult.Succeed();
                    _logger.LogInformation($"User '{username}' activated successfully");
                }
                else {
        ...
        public async Task<Result> DeactivateUserAsync(string username) {
            ...
            try {
                ...
                if (result.Succeeded) {
                    serviceResult.Succeed();
                    _logger.LogInformation($"User '{username}' deactivated successfully");
                }
                else {
        ...
        public async Task<Result> UpdateRolesAsync(string username, List<string> roles) {
            ...
            try {
                ...
                if (identityResult.Succeeded) {
                    _logger.LogWarning($"Roles for user '{username}' updated successfully. Roles: {string.Join(",", roles)}");
                    serviceResult.Succeed();
                }
                else {
                    _logger.LogWarning($"Unable to update roles for user '{username}'");
        ...
        public async Task<Result> UpdateUserDetailsAsync(UpdateUserDetailsDTO request) {
            ...
            try {
                ...
                if (!string.IsNullOrWhiteSpace(request.Surname) && applicationUser.Surname != request.Surname)
                    applicationUser.Surname = request.Surname;
                ...
                if (!string.IsNullOrWhiteSpace(request.LanguagePreference) && applicationUser.UICulture != request.LanguagePreference) {
                    applicationUser.UICulture = request.LanguagePreference;
                }

                if (applicationUser.Address == null) {
                    applicationUser.Address = new AddressEntity();
                }

                applicationUser.Address.Line1 = request.Address.Line1;
                applicationUser.Address.Line2 = request.Address.Line2;
                applicationUser.Address.City = request.Address.City;
                applicationUser.Address.Country = request.Address.Country;
                applicationUser.Address.Postcode = request.Address.PostCode;
                applicationUser.Address.Area = request.Address.Area;
                applicationUser.Address.FlatNumber = request.Address.FlatNumber;
        ...
        public async Task<Result> ConfirmUserEmailAsync(string userId) {
            Result serviceResult = new Result();
            try {
                _logger.LogInformation($"Manually confirming user email for user with id '{userId}'.");
